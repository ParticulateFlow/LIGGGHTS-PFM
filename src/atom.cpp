...
Atom::~Atom()
{
  ...
  memory->destroy(density); //NP modified C.K.
  ...
  memory->destroy(bond_hist); //NP modified C.K.
...
int Atom::count_words(const char *line)
{
  ...
  char *ptr;
  if ((ptr = strchr(copy,'#'))) *ptr = '\0'; //NP modified R.B.
...
void *Atom::extract(const char *name,int &len) //NP modified C.K. added len
{
  //NP modified C.K. all
  len = 0;
  ...
  if (strcmp(name,"rmass") == 0) return (void *) rmass; //NP modified C.K.
  if (strcmp(name,"radius") == 0) return (void *) radius; //NP modified C.K.
