...
AtomVecAngleGran::AtomVecAngleGran(LAMMPS *lmp) :
  AtomVec(lmp)
{
  molecular = 1;
...
void AtomVecAngleGran::settings(int narg, char **arg)
{
  ...
  if (narg != 8) error->all(FLERR,"Invalid atom_style angle/gran command,expecting exactly 8 arguments");

  if(strcmp(arg[0],"n_bondtypes"))
    error->all(FLERR,"Illegal atom_style angle/gran command, expecting 'n_bondtypes'");

  atom->nbondtypes = atoi(arg[1]);

  if(strcmp(arg[2],"bonds_per_atom"))
    error->all(FLERR,"Illegal atom_style angle/gran command, expecting 'bonds_per_atom'");

  atom->bond_per_atom = atoi(arg[3]);

  if(strcmp(arg[4],"n_angletypes"))
     error->all(FLERR,"Illegal atom_style angle/gran command, expecting 'n_angletypes'");

  atom->nangletypes = atoi(arg[5]);

  if(strcmp(arg[6],"angles_per_atom"))
     error->all(FLERR,"Illegal atom_style angle/gran command, expecting 'angles_per_atom'");

  atom->angle_per_atom = atoi(arg[7]);

  /*NL*/ if(screen) fprintf(screen,"atom->nbondtypes %d atom->bond_per_atom %d\n",atom->nbondtypes,atom->bond_per_atom);
  /*NL*/ if(screen) fprintf(screen,"atom->nangletypes %d atom->angle_per_atom %d\n",atom->nangletypes,atom->angle_per_atom);
}

/* ---------------------------------------------------------------------- */

void AtomVecAngleGran::init()
{
...
void AtomVecAngleGran::grow(int n)
{
  if (n == 0) nmax += DELTA;
  else nmax = n;
  atom->nmax = nmax;
  ...
  angle_type = memory->grow(atom->angle_type,nmax,atom->angle_per_atom,"atom:angle_type");
  angle_atom1 = memory->grow(atom->angle_atom1,nmax,atom->angle_per_atom,"atom:angle_atom1");
  angle_atom2 = memory->grow(atom->angle_atom2,nmax,atom->angle_per_atom,"atom:angle_atom2");
  angle_atom3 = memory->grow(atom->angle_atom3,nmax,atom->angle_per_atom,"atom:angle_atom3");
  ...
  if(0 == atom->angle_per_atom)
    error->all(FLERR,"Angular style bonded particles need angle_per_atom > 0");
...
void AtomVecAngleGran::grow_reset()
{
...
void AtomVecAngleGran::copy(int i, int j, int delflag)
{
  int k,l;
...
int AtomVecAngleGran::pack_comm(int n, int *list, double *buf,
...
int AtomVecAngleGran::pack_comm_vel(int n, int *list, double *buf,
             int pbc_flag, int *pbc)
{
  int i,j,m;
...
void AtomVecAngleGran::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecAngleGran::unpack_comm_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecAngleGran::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecAngleGran::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecAngleGran::pack_border(int n, int *list, double *buf,
           int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecAngleGran::pack_border_vel(int n, int *list, double *buf,
         int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecAngleGran::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecAngleGran::pack_border_one(int i, double *buf)
{
  buf[0] = molecule[i];
  return 1;
}
...
void AtomVecAngleGran::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecAngleGran::unpack_border_vel(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecAngleGran::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecAngleGran::unpack_border_one(int i, double *buf)
{
...
int AtomVecAngleGran::pack_exchange(int i, double *buf)
{
  int k,l;
...
int AtomVecAngleGran::unpack_exchange(double *buf)
{
  int k,l;
...
int AtomVecAngleGran::size_restart()
{
  int i;
  ...
  for (i = 0; i < nlocal; i++)
  {
    n += 13 + 2*num_bond[i] + 4*num_angle[i];
...
int AtomVecAngleGran::pack_restart(int i, double *buf)
{
  int k,l;
...
int AtomVecAngleGran::unpack_restart(double *buf)
{
  int k,l;
...
void AtomVecAngleGran::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecAngleGran::data_atom(double *coord, tagint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecAngleGran::data_atom_hybrid(int nlocal, char **values)
{
#ifdef BOND_GRAN_USE_MOLECULE_ID
...
void AtomVecAngleGran::pack_data(double **buf)
{
...
int AtomVecAngleGran::pack_data_hybrid(int i, double *buf)
{
  error->all(FLERR,"needs revision");
  buf[0] = molecule[i];
  return 1;
...
void AtomVecAngleGran::write_data(FILE *fp, int n, double **buf)
{
  error->all(FLERR,"needs revision");
  for (int i = 0; i < n; i++)
...
int AtomVecAngleGran::write_data_hybrid(FILE *fp, double *buf)
{
  error->all(FLERR,"needs revision");
...
void AtomVecAngleGran::write_restart_settings(FILE *fp)
{
  fwrite(&atom->nbondtypes,sizeof(int),1,fp);
  fwrite(&atom->bond_per_atom,sizeof(int),1,fp);
  fwrite(&atom->nangletypes,sizeof(int),1,fp);
  fwrite(&atom->angle_per_atom,sizeof(int),1,fp);
}

void AtomVecAngleGran::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    fread(&atom->nbondtypes,sizeof(int),1,fp);
    fread(&atom->bond_per_atom,sizeof(int),1,fp);
    fread(&atom->nangletypes,sizeof(int),1,fp);
    fread(&atom->angle_per_atom,sizeof(int),1,fp);
  }
  MPI_Bcast(&atom->nbondtypes,1,MPI_INT,0,world);
  MPI_Bcast(&atom->bond_per_atom,1,MPI_INT,0,world);
  MPI_Bcast(&atom->nangletypes,1,MPI_INT,0,world);
...
bigint AtomVecAngleGran::memory_usage()
{
  bigint bytes = 0.0;
  ...
  if (atom->memcheck("num__angle")) bytes += memory->usage(num_angle,nmax);
  if (atom->memcheck("angle_type")) bytes += memory->usage(angle_type,nmax,atom->angle_per_atom);
  if (atom->memcheck("angle_atom1")) bytes += memory->usage(angle_atom1,nmax,atom->angle_per_atom);
  if (atom->memcheck("angle_atom2")) bytes += memory->usage(angle_atom2,nmax,atom->angle_per_atom);
  if (atom->memcheck("angle_atom3")) bytes += memory->usage(angle_atom3,nmax,atom->angle_per_atom);
