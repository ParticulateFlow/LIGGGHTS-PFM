...
AtomVecSphere::AtomVecSphere(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  size_forward = 3;
  size_reverse = 6;
  size_border = 9;//NP formerly 8 - now also communicating density
  size_velocity = 6;
  size_data_atom = 7;
  size_data_vel = 7;
  xcol_data = 5;
...
void AtomVecSphere::copy(int i, int j, int delflag)
{
  ...
  radius[j] = radius[i];
  rmass[j] = rmass[i];
  density[j] = density[i]; //NP modified C.K.
  omega[j][0] = omega[i][0];
  omega[j][1] = omega[i][1];
  omega[j][2] = omega[i][2];
  ...
  if (radvary == 0) {
    ...
    if (pbc_flag == 0) {
      for (i = 0; i < n; i++) {
        ...
        buf[m++] = radius[j];
        buf[m++] = rmass[j];
        buf[m++] = density[j]; //NP modified C.K.
      }
    } else {
      if (domain->triclinic == 0) {
        dx = pbc[0]*domain->xprd;
...
void AtomVecSphere::unpack_comm(int n, int first, double *buf)
{
  ...
  if (radvary == 0) {
    ...
    for (i = first; i < last; i++) {
      ...
      radius[i] = buf[m++];
      rmass[i] = buf[m++];
      density[i] = buf[m++]; //NP modified C.K.
    }
  }
}
...
int AtomVecSphere::pack_exchange(int i, double *buf)
{
  ...
  buf[m++] = radius[i];
  buf[m++] = rmass[i];
  buf[m++] = density[i]; //NP modified C.K.
  buf[m++] = omega[i][0];
  buf[m++] = omega[i][1];
  buf[m++] = omega[i][2];
...
int AtomVecSphere::unpack_exchange(double *buf)
{
  ...
  radius[nlocal] = buf[m++];
  rmass[nlocal] = buf[m++];
  density[nlocal] = buf[m++]; //NP modified C.K.
...
int AtomVecSphere::size_restart()
{
  ...
  int nlocal = atom->nlocal;
  int n = 17 * nlocal; //NP modified C.K.
...
void AtomVecSphere::create_atom(int itype, double *coord)
{
  ...
  if (domain->dimension == 2) rmass[nlocal] = MY_PI *
      radius[nlocal]*radius[nlocal]*density[nlocal]; //NP modified GM
  else rmass[nlocal] = 4.0*MY_PI/3.0 *
    radius[nlocal]*radius[nlocal]*radius[nlocal] * density[nlocal]; //NP modified GM
  omega[nlocal][0] = 0.0;
  omega[nlocal][1] = 0.0;
  omega[nlocal][2] = 0.0;
...
void AtomVecSphere::pack_data(double **buf,int tag_offset)
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]+tag_offset).d; //NP modified C.K.
    buf[i][1] = ubuf(type[i]).d;
    buf[i][2] = 2.0*radius[i];
    if (radius[i] == 0.0) buf[i][3] = rmass[i];
    else
...
void AtomVecSphere::pack_vel(double **buf,int tag_offset) //NP modified C.K.
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
