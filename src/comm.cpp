...
void Comm::init()
{
  ...
  //NP for fix multisphere, otherwise no reverse comm possible
  //NP if (force->newton == 0) maxreverse = 0;
...
void Comm::setup()
{
  ...
  //NP modified C.K.
  //NP  this is needed b/c proc that owns body needs to have full
  //NP  info about all particles in body - thus need to extend corona
  //NP so let fix rigid multisphere and fix template multisphere do this
    ...
    for (ineed = 0; ineed < 2*maxneed[dim]; ineed++) {
      ...
      if (ineed % 2 == 0) {
        ...
        if (style == SINGLE) {
          ...
          slabhi[iswap] = sublo[dim] + (use_gran_opt() ? (cutghost[dim] / 2. + neighbor->skin/2.) : cutghost[dim]); //NP modified C.K. take advantage of radius information
        } else {
          for (i = 1; i <= ntypes; i++) {
        ...
        if (style == SINGLE) {
          slablo[iswap] = subhi[dim] - (use_gran_opt() ? (cutghost[dim] / 2. + neighbor->skin/2.) : cutghost[dim]); //NP modified C.K. take advantage of radius information
