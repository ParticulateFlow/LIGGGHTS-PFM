...
void ComputeKEAtom::init()
{
  ...
  fix_ms =  static_cast<FixMultisphere*>(modify->find_fix_style("multisphere",0)); //NP modified C.K.
}
...
void ComputeKEAtom::compute_peratom()
{
  ...
  if (rmass)
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit && (!fix_ms || fix_ms->belongs_to(i) < 0)) { //NP modified C.K.
