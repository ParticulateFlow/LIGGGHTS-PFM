...
enum{X,V,F,RHO,P,COMPUTE,FIX,VARIABLE}; //NP modified C.K.
enum{PERATOM,LOCAL};
  ...
  Compute(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    ...
    } else if (strcmp(arg[iarg],"rho") == 0) { //NP modified C.K.
      which[nvalues] = RHO;
      argindex[nvalues++] = 0;
...
double ComputeReduce::compute_one(int m, int flag)
{
  ...
  } else if (which[m] == RHO) { //NP modified C.K.
    double *rho = atom->rho;
    if (flag < 0) {
      for (i = 0; i < nlocal; i++)
        if (mask[i] & groupbit) combine(one,rho[i],i);
    } else one = rho[flag];
  } else if (which[m] == P) { //NP modified C.K.
    double *p = atom->p;
    if (flag < 0) {
      for (i = 0; i < nlocal; i++)
