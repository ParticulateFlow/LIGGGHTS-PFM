...
void Domain::reset_box()
{
  ...
  if (nonperiodic == 2) {
    ...
    //NP modified C.K.
    //NP give modifyy the chance to extend the box
    //NP this is for meshes
  ...
  set_global_box();
  //NP modified C.K.
  /*NL*/if(decide_loadbalance())
  /*NL*/   lbalance->loadbalance_local_boxes();
  /*NL*/ else
           set_local_box();
...
/*NP
  ...
  + should not start with set_local_box, but with last box border
  + should check maximum border change in order to prevent atoms skipping stencil
  + should do loadbalancing out of verlet also if there is no neigh build
*/


/*NL*/int Domain::decide_loadbalance()
/*NL*/{
/*NL*/   if (lbalance) return 1;
/*NL*/   return 0;
/*NL*/}
...
void Domain::box_too_small_check()
{
  ...
  double delx,dely,delz,rsq; //NP modified R.B.
