...
void ExtractSurface::command(int narg, char **arg)
{
  ...
  #endif
      char *newfilename = new char[strlen(filename)-strlen(suffix)+21];
      strncpy(newfilename, filename, strlen(filename)-strlen(suffix));
      newfilename[strlen(filename)-strlen(suffix)] = 0;
