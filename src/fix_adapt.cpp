  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      ...
      if (strcmp(arg[iarg+1],"diameter") == 0) {
        ...
        //NP modified C.K.
        //NP so that neigh list and forward comm is done correctly
...
void FixAdapt::change_settings()
{
  ...
  for (int m = 0; m < nadapt; m++) {
    ...
    bool is_atomstyle = (input->variable->atomstyle(ad->ivar)) == 1; //NP modified C.K.
      ...
      if (ad->aparam == DIAMETER) {
        ...
        if (mflag == 0) {
          for (i = 0; i < nlocal; i++)
            if (mask[i] & groupbit)
            {
              if (is_atomstyle) value = fppat->vector_atom[i];  //NP modified C.K.
              radius[i] = 0.5*value;
            }
        } else {
          for (i = 0; i < nlocal; i++)
