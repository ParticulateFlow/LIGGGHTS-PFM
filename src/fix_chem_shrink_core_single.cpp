...
void FixChemShrinkCoreSingle::post_create()
{
    ...
    char* fixname = new char [strlen("porosity_")+strlen(group->names[igroup])+1];
    strcpy(fixname,"porosity_");
    strcat(fixname,group->names[igroup]);
    fix_porosity_ = static_cast<FixPropertyGlobal*>(modify->find_fix_property(fixname, "property/global", "vector", 0, 0, this->style, false));
    if (fix_porosity_ == NULL) {
        ...
        fixarg[8]="0.0";
        fixarg[9]="0.0";
        fix_porosity_ = modify->add_fix_property_global(10,const_cast<char**>(fixarg),style);
