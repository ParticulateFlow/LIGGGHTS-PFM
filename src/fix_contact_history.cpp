...
void FixContactHistory::initial_integrate(int dummy)
{
    ...
    for(int i = 0; i < nlocal; i++)
    {
        if(atom->tag[i] == 2723)
        {
            fprintf(screen,"tag 2723 ilocal %d  npartner %d\n",i,npartner[i]);
            for(int j = 0; j < npartner[i]; j++)
                fprintf(screen,"    partner %d: tri id %d\n",j,partner[i][j]);
        }
    }*/
}
...
void FixContactHistory::setup_pre_exchange()
{
  if(is_pair)
  {
    /*NL*/ //fprintf(screen,"(*computeflag) %d\n",(*computeflag));
...
{
    //NP dont need to copy since for meshes info is always stored here
    if(is_pair)
        pre_exchange_pair();
...
//       Check if we need to consider non-contacting particles
       ...
       { 
        ...
        partner[i][m] = tag[j];
        for (d = 0; d < dnum; d++) {
            contacthistory[i][m][d] = hist[d]; //NP modified C.K.
        }
        npartner[i]++;
        if (j < nlocal) {
            if (npartner[j] >= maxtouch) grow_arrays_maxtouch(atom->nmax); //NP modified C.K.

            m = npartner[j];
            partner[j][m] = tag[i];
            for (d = 0; d < dnum; d++) {
               if(newtonflag[d]) contacthistory[j][m][d] = -hist[d]; //NP modified C.K.
               else              contacthistory[j][m][d] =  hist[d]; //NP modified C.K.
            }
            npartner[j]++;
        }
      }
...
double FixContactHistory::memory_usage()
{
  ...
  bytes += nmax*maxtouch * sizeof(int);  //NP modified C.K.
  bytes += nmax*maxtouch * dnum * sizeof(double); //NP modified C.K.
  return bytes;
}
...
void FixContactHistory::grow_arrays_maxtouch(int nmax)
{
  ...
  //NP manually copy array values
  //NP zeroize new values
  for (int i = 0; i < nmax; i++)
  {
      ...
      /*NP TODO unitialized values here
      for(int j = maxtouch; j < maxtouch+delta; j++)
      {
          partner_g[i][j] = -1;
          delflag_g[i][j] = 0;
          for(int k = 0; k < dnum; k++)
...
{
    /*NL*///fprintf(screen,"set_arrays, %d\n",i);
    npartner[i] = 0;
}
...
void FixContactHistory::write_restart(FILE *fp)
{
  ...
  //NP copy values to atom arrays so can be stored
  pre_exchange();
}
...
void FixContactHistory::cleanUpContacts()
{
    ...
    for(int i = 0; i < nlocal; i++)
    {
        ...
        while(j < npartner[i])
        {
            ...
            if(delflag[i][j])
            {
                /*NL*/ //fprintf(screen,"deleting contact for i %d, npartner %d\n",i,npartner[i]);
