...
FixStyle(contacthistory,FixContactHistory) //NP for use by pair Style Neigh class references to this fix

FixStyle(contacthistory/mesh,FixContactHistory) //NP for use by mesh
...
class FixContactHistory : public Fix {
 ...
 private:
  ...
  void grow_arrays_maxtouch(int);//NP modified C.K.

  bool **delflag;               //NP specific for mesh handling, history transfer
  ...
  //NP modified C.K.
  //NP modularize dnum and Newton flag
  int dnum;
  int *newtonflag;
  char **history_id;
  int laststep;
