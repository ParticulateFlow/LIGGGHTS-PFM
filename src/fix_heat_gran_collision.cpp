...
#include "fix_heat_gran_collision.h"
...
FixHeatGranColl::FixHeatGranColl(class LAMMPS *lmp, int narg, char **arg) :
  ...
  correction_factor_(1.0)
{
  ...
  while(iarg_ < narg && hasargs)
  {
    ...
    if(strcmp(arg[iarg_],"contact_area") == 0) {
      ...
      else error->fix_error(FLERR,this,"expecting 'overlap' or 'constant' after 'contact_area'");
      iarg_ += 2;
      hasargs = true;
    } else if(strcmp(arg[iarg_],"compute_correction") == 0) {
      if (iarg_+2 > narg) error->fix_error(FLERR,this,"not enough arguments for keyword 'compute_correction'");
      if(strcmp(arg[iarg_+1],"yes") == 0)
        compute_correction_flag_ = 1;
      else if(strcmp(arg[iarg_+1],"no") == 0)
        compute_correction_flag_ = 0;
      else error->fix_error(FLERR,this,"expecting 'yes' or 'no' after 'compute_correction'");     
      iarg_ += 2;
      hasargs = true;
    } else if(strcmp(arg[iarg_],"correction_factor") == 0) {
      if (iarg_+2 > narg) error->fix_error(FLERR,this,"not enough arguments for keyword 'correction_factor'");
      correction_factor_ = force->numeric(FLERR,arg[iarg_+1]);
      if (correction_factor_ <= 0.)
          error->fix_error(FLERR,this,"'correction_factor' value must be > 0");
      iarg_ += 2;
      hasargs = true;
    } else if(strcmp(style,"heat/gran/collision") == 0)
        error->fix_error(FLERR,this,"unknown keyword");
  }

  if(CONDUCTION_CONTACT_AREA_OVERLAP != area_calculation_mode_ && 1 == compute_correction_flag_)
    error->fix_error(FLERR,this,"can use 'area_correction' only for 'contact_area = overlap'");
}

/* ---------------------------------------------------------------------- */

FixHeatGranColl::~FixHeatGranColl()
{
...
void FixHeatGranColl::post_create()
{
...
void FixHeatGranColl::pre_delete(bool unfixflag)
{
  ...
  for(int i=1;i< max_type+1; i++)
      for(int j=1;j<max_type+1;j++)
      {
          ...
          if(conductivity_[i-1] < 0.)
            error->all(FLERR,"Fix heat/gran/collision: Thermal conductivity must not be < 0");
    
  if (capacity_) delete []capacity_;
  capacity_ = new double[max_type];
  fix_capacity_ =
    static_cast<FixPropertyGlobal*>(modify->find_fix_property("thermalCapacity","property/global","peratomtype",max_type,0,style));

  // pre-calculate capacity for possible contact material combinations
  for(int i=1;i< max_type+1; i++)
      for(int j=1;j<max_type+1;j++)
      {
          capacity_[i-1] = fix_capacity_->compute_vector(i-1);
          if(capacity_[i-1] < 0.)
            error->all(FLERR,"Fix heat/gran/collision: Thermal capacity must not be < 0");
     }

  // calculate heat transfer correction

  if(compute_correction_flag_)
  {
    ...
    correction_factor_ = static_cast<FixPropertyGlobal*>(modify->find_fix_property("youngsModulusOriginal","property/global","peratomtype",max_type,0,style))->get_array_modified();
  }
...
void FixHeatGranColl::post_force(int vflag)
{
...
void FixHeatGranColl::cpl_evaluate(ComputePairGranLocal *caller)
{
  if(caller != cpl) error->all(FLERR,"Illegal situation in FixHeatGranColl::cpl_evaluate");
...
void FixHeatGranColl::post_force_eval(int vflag,int cpl_flag)
{
  double hc,contactArea,delta_n,flux,dirFlux[3];
  int i,j,ii,jj,inum,jnum;
  double xtmp,ytmp,ztmp,delx,dely,delz;
  double radi,radj,densi,densj,radsum,rsq,r,tcoi,tcoj,tcapi,tcapj;
  ...
  if (strcmp(force->pair_style,"hybrid")==0)
    error->warning(FLERR,"Fix heat/gran/collision implementation may not be valid for pair style hybrid");
  if (strcmp(force->pair_style,"hybrid/overlay")==0)
    error->warning(FLERR,"Fix heat/gran/collision implementation may not be valid for pair style hybrid/overlay");
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	tcapi = capacity_[type[i]-1];
        tcapj = capacity_[type[j]-1];	
	zi = densi*tcoi*tcapi;
	zj = densj*tcoj*tcapj;
	
        if (zi < SMALL || zj < SMALL) hc = 0.;
        else
	{
	    zi = 1.0/sqrt(zi);
	    zj = 1.0/sqrt(zj)
	    hc = contactArea/((zi+zj)*sqrt(collision_time_));
            hc *= correction_factor_;
	}
...
void FixHeatGranColl::register_compute_pair_local(ComputePairGranLocal *ptr)
{
   /*NL*///fprintf(screen,"FixHeatGran::register_compute_pair_local, ptr->id %s\n",ptr->id);

   if(cpl != NULL)
      error->all(FLERR,"Fix heat/gran/collision allows only one compute of type pair/local");
   cpl = ptr;
}

void FixHeatGranColl::unregister_compute_pair_local(ComputePairGranLocal *ptr)
{
   /*NL*///fprintf(screen,"FixHeatGran::unregister_compute_pair_local\n");
   if(cpl != ptr)
       error->all(FLERR,"Illegal situation in FixHeatGranColl::unregister_compute_pair_local");
   cpl = NULL;
}
