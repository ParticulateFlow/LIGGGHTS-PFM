...
void FixMassflowMeshFaceUniverse::post_create()
{
    ...
    if (comm->me == 0 && send_to_world_ >= 0) {
      std::vector<int> keys;
      mapkeys2vector(faceid2index_, keys);//, vals);
...
void FixMassflowMeshFaceUniverse::pre_delete(bool unfixflag)
{
    if (unfixflag)
    {
...
void FixMassflowMeshFaceUniverse::init()
{
...
void FixMassflowMeshFaceUniverse::setup(int /*vflag*/)
{
}

/* ---------------------------------------------------------------------- */

int FixMassflowMeshFaceUniverse::setmask()
{
    int mask = 0;
    mask |= POST_INTEGRATE;
...
void FixMassflowMeshFaceUniverse::post_integrate()
{
        ...
        for(int i=0; i<nfaceids; ++i)
        {
            if(nparticles_face_this_all[i] > 0)
            {
                ...
                for(int p = 0; p < nparticles_face_this_all[i]; ++p)
                {
                    ...
                    distributions_faces_[cpts].resize(nfaceids, 0.);
                    distributions_faces_[cpts][i] += cg3_;
                }
            }
        }
    ...
    if((couple_every_ > 0) &&  (update->ntimestep > 0) && ((update->ntimestep-1) % couple_every_ == 0))
    {
        if(comm->me == 0) {
            ...
            {
                int send_data_size = distributions_faces_.size()*(3+nfaceids);
...
void FixMassflowMeshFaceUniverse::pre_exchange()
{
    if(delete_atoms_)
    {
...
void FixMassflowMeshFaceUniverse::write_restart(FILE *fp)
{
  int n = 0;
  double list[6];
  list[n++] = mass_;
...
void FixMassflowMeshFaceUniverse::restart(char *buf)
{
  int n = 0;
...
double FixMassflowMeshFaceUniverse::compute_scalar()
{
...
double FixMassflowMeshFaceUniverse::compute_vector(int index)
{
    if(reset_t_count_)
    {
        delta_t_ = t_count_;
...
double FixMassflowMeshFaceUniverse::compute_array(int i, int j)
{
  if(reset_t_count_)
  {
      delta_t_ = t_count_;
...
double FixMassflowMeshFaceUniverse::compute_array_by_id(int face_id, int j)
{
...
bool FixMassflowMeshFaceUniverse::confirm_classification(int ibody, int iPart, int tri, int current_side)
{
