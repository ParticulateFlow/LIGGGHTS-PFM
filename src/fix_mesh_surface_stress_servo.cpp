...
void FixMeshSurfaceStressServo::limit_vel()
{
  ...
  /*NL*/ //if(screen) fprintf(screen,"Test for velocity: ctrl_op_max_ = %g; vmag = %g\n",ctrl_op_max_,vmag);
    ...
    double *delta, int iTri, double *v_wall)
{
  ...
  //NP TODO: Calculate the transformed vector and calculate the right circle
...
int FixMeshSurfaceStressServo::modify_param(int narg, char **arg)
{
  if (strcmp(arg[0],"integrate") == 0) {
    ...
    if (strstr(arg[1],"v_") == arg[1]) {
      ...
      //NP TODO: Those parts are from init(). Should we just run init() again?
      // check variables
      if (sp_str_) {
        sp_var_ = input->variable->find(sp_str_);
        if (sp_var_ < 0)
