...
FixPropertyAtomLammps::FixPropertyAtomLammps(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
FixPropertyAtomLammps::~FixPropertyAtomLammps()
{
...
int FixPropertyAtomLammps::setmask()
{
  int mask = 0;
  return mask;
}
...
void FixPropertyAtomLammps::init()
{
...
void FixPropertyAtomLammps::read_data_section(char *keyword, int n, char *buf)
{
  int j,m;
  tagint itag;
  char *next;
...
bigint FixPropertyAtomLammps::read_data_skip_lines(char *keyword)
{
  return atom->natoms;
}
...
void FixPropertyAtomLammps::write_data_section_size(int mth, int &nx, int &ny)
{
  nx = atom->nlocal;
  ny = nvalue + 1;
}
...
void FixPropertyAtomLammps::write_data_section_pack(int mth, double **buf)
{
  int i;
...
void FixPropertyAtomLammps::write_data_section_keyword(int mth, FILE *fp)
{
...
void FixPropertyAtomLammps::write_data_section(int mth, FILE *fp,
...
double FixPropertyAtomLammps::memory_usage()
{
  double bytes = 0.0;
  for (int m = 0; m < nvalue; m++) {
...
void FixPropertyAtomLammps::grow_arrays(int nmax)
{
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE) {
...
void FixPropertyAtomLammps::copy_arrays(int i, int j, int delflag)
{
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE)
...
int FixPropertyAtomLammps::pack_border(int n, int *list, double *buf)
{
  int i,j,k;
...
int FixPropertyAtomLammps::unpack_border(int n, int first, double *buf)
{
  int i,k,last;
...
int FixPropertyAtomLammps::pack_exchange(int i, double *buf)
{
  for (int m = 0; m < nvalue; m++) {
...
int FixPropertyAtomLammps::unpack_exchange(int nlocal, double *buf)
{
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE)
...
int FixPropertyAtomLammps::pack_restart(int i, double *buf)
{
  buf[0] = nvalue+1;
...
void FixPropertyAtomLammps::unpack_restart(int nlocal, int nth)
{
...
int FixPropertyAtomLammps::maxsize_restart()
{
  return nvalue+1;
}
...
int FixPropertyAtomLammps::size_restart(int nlocal)
{
  return nvalue+1;
}
