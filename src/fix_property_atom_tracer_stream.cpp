...
{
    //NP ensure end_of_step() FixPropertyAtomTracer is NOT called of base class
...
void FixPropertyAtomTracerStream::mark_tracers(int ilo, int ihi)
{
    ...
    //NP get list of all particles with release step > step_
    ...
    //NP allreduce container in multiproc case
    //NP do nothing in single proc case
    ...
    //NP count first n_tracer_ eligible particles with step0 <= step
    //NP mark them if I own them
    ...
    /*NL*///error->one(FLERR,"TODO here; auch  überhang von vorigem TS");
}
