...
#define myAtof lmp->force->numeric //NP force->numeric returns float value from string (alternative to atof)
#define myAtoi lmp->force->inumeric //NP force->inumeric returns int value from string (alternative to atoi)
  ...
  Fix(lmp, narg, arg)
{
    ...
    /*NL*/ //if ((data_style == FIXPROPERTY_GLOBAL_MATRIX)&&(narg <10)) error->all(FLERR,"Illegal fix property/global command, not enough arguments for style 'matrix'");
    ...
    //assign values
    nvalues = narg - 5 - darg;
    /*NL*/ //if ((nvalues==1) && (data_style==1)) error->all(FLERR,"Error in fix property/global: Number of default values provided not consistent with vector style. Provide more than 1 value or use style 'scalar'");
...
{
    //NP since this would overrite old values
