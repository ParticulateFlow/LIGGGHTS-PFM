/* ----------------------------------------------------------------------
   LIGGGHTS - LAMMPS Improved for General Granular and Granular Heat
   Transfer Simulations

   LIGGGHTS is part of the CFDEMproject
   www.liggghts.com | www.cfdem.com

   Christoph Kloss, christoph.kloss@cfdem.com
   Copyright 2009-2012 JKU Linz
   Copyright 2012-     DCS Computing GmbH, Linz

   LIGGGHTS is based on LAMMPS
   LAMMPS - Large-scale Atomic/Molecular Massively Parallel Simulator
   http://lammps.sandia.gov, Sandia National Laboratories
   Steve Plimpton, sjplimp@sandia.gov

   This software is distributed under the GNU General Public License.

   See the README file in the top-level directory.
------------------------------------------------------------------------- */

/* ----------------------------------------------------------------------
   Contributing authors for original version: Leo Silbert (SNL), Gary Grest (SNL)
------------------------------------------------------------------------- */

#include "math.h"
#include "stdlib.h"
#include "string.h"
#include "fix_wall_gran_jkr_history.h"
#include "pair_gran_jkr_history.h"
#include "atom.h"
#include "domain.h"
#include "update.h"
#include "force.h"
#include "pair.h"
#include "modify.h"
#include "respa.h"
#include "memory.h"
#include "error.h"
#include "fix_rigid.h"
#include "fix_property_global.h"
#include "mech_param_gran.h"

using namespace LAMMPS_NS;
using namespace FixConst;

#define BIG 1.0e20

#define sqrtFiveOverSix 0.91287092917527685576161630466800355658790782499663875

/* ---------------------------------------------------------------------- */

FixWallGranJKRHistory::FixWallGranJKRHistory(LAMMPS *lmp, int narg, char **arg) :
  FixWallGranHookeHistory(lmp, narg, arg)
{

}

/* ---------------------------------------------------------------------- */

void FixWallGranJKRHistory::init_granular()
{
  //get material properties
  Yeff = ((PairGranJKRHistory*)pairgran_)->Yeff;
  Geff = ((PairGranJKRHistory*)pairgran_)->Geff;
  betaeff = ((PairGranJKRHistory*)pairgran_)->betaeff;
  veff = ((PairGranJKRHistory*)pairgran_)->veff;
  cohEnergyDens = ((PairGranJKRHistory*)pairgran_)->cohEnergyDens;
  coeffRestLog = ((PairGranJKRHistory*)pairgran_)->coeffRestLog;
  coeffFrict = ((PairGranJKRHistory*)pairgran_)->coeffFrict;
  coeffRollFrict = ((PairGranJKRHistory*)pairgran_)->coeffRollFrict;
  //coeffRollVisc = ((PairGranJKRHistory*)pairgran_)->coeffRollVisc;

  //need to check properties for rolling friction and cohesion energy density here
  //since these models may not be active in the pair style
  //NP yet we write into the memory of the pair style
  FixPropertyGlobal *coeffRollFrict1, *coeffRollVisc1, *cohEnergyDens1;
  int max_type = pairgran_->mpg->max_type();
  if(rollingflag)
    coeffRollFrict1=static_cast<FixPropertyGlobal*>(modify->find_fix_property("coefficientRollingFriction","property/global","peratomtypepair",max_type,max_type,style));
  if(rollingflag == 2) // epsd model
    coeffRollVisc1=static_cast<FixPropertyGlobal*>(modify->find_fix_property("coefficientRollingViscousDamping","property/global","peratomtypepair",max_type,max_type,force->pair_style));
  if(cohesionflag)
    cohEnergyDens1=static_cast<FixPropertyGlobal*>(modify->find_fix_property("cohesionEnergyDensity","property/global","peratomtypepair",max_type,max_type,style));

  //pre-calculate parameters for possible contact material combinations
  for(int i=1;i< max_type+1; i++)
  {
      for(int j=1;j<max_type+1;j++)
      {
          if(rollingflag) coeffRollFrict[i][j] = coeffRollFrict1->compute_array(i-1,j-1);
          if(rollingflag == 2) coeffRollVisc[i][j] = coeffRollVisc1->compute_array(i-1,j-1);
          if(cohesionflag) cohEnergyDens[i][j] = cohEnergyDens1->compute_array(i-1,j-1);
      }
  }
}

/* ----------------------------------------------------------------------
   contact model parameters derived for hertz model //NP modified C.K.
------------------------------------------------------------------------- */

inline void FixWallGranJKRHistory::deriveContactModelParams(int ip, double deltan,
    double meff_wall, double &kn, double &kt, double &gamman, double &gammat, double &xmu,double &rmu, double &vnnr)  //NP modified C.K.
{
/*NP
    char testmsg[200];
    sprintf(testmsg,"Y=%f, v=%f, cr=%f, cf=%f\n",Y->compute_vector(0),v->compute_vector(0),coeffRest->compute_array(0,0),coeffFrict->compute_array(0,0));
    error->warning(testmsg);
*/

    int itype = atom->type[ip];
    double sqrtval = sqrt(atom->radius[ip]*deltan);

    double Sn=2.*(Yeff[itype][atom_type_wall_])*sqrtval;
    double St=8.*(Geff[itype][atom_type_wall_])*sqrtval;

    kn=4./3.*Yeff[itype][atom_type_wall_]*sqrtval;
    kt=St;

    gamman=-2.*sqrtFiveOverSix*betaeff[itype][atom_type_wall_]*sqrt(Sn*meff_wall);
    gammat=-2.*sqrtFiveOverSix*betaeff[itype][atom_type_wall_]*sqrt(St*meff_wall);

    xmu=coeffFrict[itype][atom_type_wall_];
    if(rollingflag)rmu=coeffRollFrict[itype][atom_type_wall_];

    if (dampflag == 0) gammat = 0.0;

    /*NL*/ //if(itype == 2 || itype == 3) fprintf(screen,"wall yeff %f geff %f betaeff %f\n",mpg->Yeff[itype][atom_type_wall],mpg->Geff[itype][atom_type_wall],mpg->betaeff[itype][atom_type_wall]);
    /*NL*/ //if(itype == 2 || itype == 3) error->all(FLERR,"end");

/*NP
    fprintf(screen,"ip=%d, sqrtval=%f, deltan=%f, kn=%f, kt=%f, gamman=%f, gammat=%f\n", ip,sqrtval,deltan,kn,kt,gamman,gammat);
*/
    // convert Kn and Kt from pressure units to force/distance^2
    kn /= force->nktv2p;
    kt /= force->nktv2p;

    return;
}
