...
LAMMPS::LAMMPS(int narg, char **arg, MPI_Comm communicator)
{
  ...
  int screenflag = 0;
  int logflag = 0;
  int thermoflag = 0; //NP modified C.K.
  ...
  char *rfile = NULL;
  char *dfile = NULL;
  int tag_offset = 0; //NP modified C.K.

  wedgeflag = false; //NP modified C.K.
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"-partition") == 0 ||
    ...
    } else if (strcmp(arg[iarg],"-uid") == 0) { //NP modified C.K.
      if (iarg+2 > narg)
        error->universe_all(FLERR,"Invalid command-line argument");
      universe->id(arg[iarg+1]);
      iarg += 2;
    } else if (strcmp(arg[iarg],"-thermo") == 0) { //NP modified C.K.
      if (iarg+2 > narg)
        error->universe_all(FLERR,"Invalid command-line argument");
      thermoflag = iarg+1;
      iarg += 2;
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"-domain") == 0) { //NP modified C.K.
      if (iarg+2 > narg)
        error->universe_all(FLERR,"Invalid command-line argument");
  ...
  if (universe->me == 0) {
    ...
    if (logflag == 0) {
      universe->ulogfile  = fopen("log.liggghts","w");  //NP modified C.K.
      if(thermoflag > 0) universe->uthermofile = fopen(arg[thermoflag],"w");  //NP modified C.K.
      if (universe->ulogfile == NULL)
        error->universe_warn(FLERR,"Cannot open log.liggghts for writing");   //NP modified C.K.
      if (thermoflag > 0 && universe->uthermofile == NULL)                                         //NP modified C.K.
        error->universe_warn(FLERR,"Cannot open log.liggghts.thermo for writing");
    } else if (strcmp(arg[logflag],"none") == 0) {
      universe->ulogfile = NULL;
      universe->uthermofile = NULL; //NP modified C.K.
    } else {
  ...
  if (universe->existflag == 0) {
    ...
    thermofile = universe->uthermofile; //NP modified C.K.
...
LAMMPS::~LAMMPS()
{
  ...
  if (universe->nworlds == 1) {
    ...
    if (thermofile) fclose(thermofile); //NP modified C.K.
  } else {
    if (screen && screen != stdout) fclose(screen);
    if (logfile) fclose(logfile);
    if (thermofile) fclose(thermofile); //NP modified C.K.
    if (universe->ulogfile) fclose(universe->ulogfile);
    if (universe->uthermofile) fclose(universe->uthermofile); //NP modified C.K.
  }
...
void LAMMPS::create()
{
  ...
  {
    if (wedgeflag) domain = new DomainWedge(this); //NP modified C.K.
    else domain = new Domain(this);
  }
#endif

  domain->is_wedge = wedgeflag; //NP modified C.K.
  if(wedgeflag && cuda)
