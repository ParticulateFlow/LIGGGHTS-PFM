...
void Neighbor::bond_all()
{
  ...
  double ***bond_hist = atom->bond_hist;  //NP modified C.K.
  int *tag = atom->tag;
  int newton_bond = force->newton_bond;
  int n_bondhist = atom->n_bondhist;  //NP modified C.K.
    ...
    for (m = 0; m < num_bond[i]; m++) {
      ...
      if (newton_bond || i < atom1) {
        if (nbondlist == maxbond) {
          maxbond += BONDDELTA;
          memory->grow(bondlist,maxbond,4,"neighbor:bondlist");  //NP modified C.K.
          if(atom->n_bondhist)
            memory->grow(bondhistlist,maxbond,atom->n_bondhist,"neighbor:bondhistlist");  //NP modified C.K.
        }
        bondlist[nbondlist][0] = i;
        bondlist[nbondlist][1] = atom1;
        bondlist[nbondlist][2] = bond_type[i][m];
        bondlist[nbondlist][3] = 0; //NP modified C.K.
        if(n_bondhist) { //NP modified C.K.
