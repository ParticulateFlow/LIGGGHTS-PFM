...
Neighbor::Neighbor(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  every = 1;
  delay = 10;
  contactDistanceFactor = 1.0; //NP modified C.K.
  dist_check = 1;
  pgsize = 100000;
  oneatom = 2000;
  binsizeflag = 0;
  ...
  maxbond = 0;
  bondlist = NULL;
  bondhistlist = NULL; //NP modified C.K.
  maxangle = 0;
  anglelist = NULL;
  maxdihedral = 0;
  dihedrallist = NULL;
...
Neighbor::~Neighbor()
{
  ...
  memory->destroy(rhold); //NP modified C.K.

  memory->destroy(binhead);
  memory->destroy(bins);

  if(mlg) delete mlg; //NP modified C.K.
  ...
  memory->destroy(bondhistlist); //NP modified C.K.
...
void Neighbor::init()
{
  ...
  //NP modified C.K.
  //NP correct cutneighmin for gran case
  if(atom->radius_flag) {
    double maxrd,minrd;
  ...
  if (dist_check) {
    if (maxhold == 0) {
      ...
      memory->create(rhold,maxhold,"neigh:rhold"); //NP modified C.K.
    }
  }
  ...
  if (atom->molecular && atom->nbonds && maxbond == 0) {
    ...
    memory->create(bondlist,maxbond,4,"neigh:bondlist");   //NP modified C.K.
    //NP modified C.K.
    if(atom->n_bondhist)
...
void Neighbor::choose_build(int index, NeighRequest *rq)
{
  ...
  if (rq->omp == 0) {
    ...
    if (rq->copy) pb = &Neighbor::copy_from;
      ...
      } else if (style == MULTI) { //NP modified C.K.
        if (0) {}
...
void Neighbor::choose_stencil(int index, NeighRequest *rq)
{
  ...
  if (rq->skip || rq->copy || rq->half_from_full) sc = NULL;
    ...
    if (style == BIN) {
      ...
      else if (rq->newton == 0) {  //NP modified C.K. end ADD ELSE
        if (newton_pair == 0) {
          if (dimension == 2)
...
int Neighbor::check_distance()
{
  ...
  double **x = atom->x;
  double *radius = atom->radius; //NP modified C.K.
  int nlocal = atom->nlocal;
  int radvary_flag = atom->radvary_flag; //NP modified C.K.
  if (includegroup) nlocal = atom->nfirst;

  int flag = 0;
  if(radvary_flag == 0) //NP modified C.K.
  {
...
void Neighbor::set(int narg, char **arg)
{
  ...
  skin = force->cg()*force->numeric(FLERR,arg[0]); //NP modified C.K.
...
void Neighbor::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"every") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"contact_distance_factor") == 0) { //NP modified C.K.
      if (iarg+2 > narg) error->all(FLERR,"Illegal neigh_modify command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal neigh_modify command");
      binsize_user = force->cg()*force->numeric(FLERR,arg[iarg+1]); //NP modified C.K.
...
bigint Neighbor::memory_usage()
{
  ...
  bytes += memory->usage(rhold,maxhold); //NP modified C.K.
