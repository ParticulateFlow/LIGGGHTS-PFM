...
Pair::Pair(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  list = listhalf = listfull = listgranhistory = listinner = listmiddle = listouter = NULL; //NP modified C.K.
...
void Pair::reinit()
{
  ...
  for (int i = 1; i <= atom->ntypes; i++) //NP modified R.B.
    for (int j = i; j <= atom->ntypes; j++) { //NP modified R.B.
      init_one(i,j);
      if (tail_flag) {
        etail += etail_ij;
        ptail += ptail_ij;
...
void Pair::init_tables_disp(double cut_lj_global)
{
  ...
  double f_tmp,e_tmp; //NP modified R.B.
