...
void PairGran::init_style()
{
  ...
  //NP calculate total # of history values

  dnum_all = dnum_pairgran;

  //NP look for extra dnums requested by fixes
  int dnum_extra = 0;
  nfix = modify->nfix;
  ...
  //NP store which index belongs to which fix
  //NP first dnum values belong to pair and wall
  ...
  if(history)
  {
    if (!fix_history)
    {
        //NP dnum already set in constructor of derived class
  ...
  //NP check for Fix pour and set pour_type and pour_maxdiam ... for all pour commands!
...
double PairGran::init_one(int i, int j)
{
  ...
  //NP add contactHistoryFactor
  //NP is thus added to pair->cutforce and pair->cutsq via Pair::init_one()
...
void PairGran::compute_pgl(int eflag, int vflag)
{
  ...
  //NP computeflag_ stores if compute has been executed since last pre_exchange()
  //NP so must reset to value before
...
------------------------------------------------------------------------- */
void PairGran::compute_sp(LCM::CollisionData &cdata, LCM::ForceData &i_forces, LCM::ForceData &j_forces)
{
