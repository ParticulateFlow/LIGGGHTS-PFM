...
PairStyle(gran/jkr/history,PairGranJKRHistory)

#else

#ifndef LMP_PAIR_GRAN_JKR_HISTORY_H
#define LMP_PAIR_GRAN_JKR_HISTORY_H
...
class PairGranJKRHistory : public PairGran {
 ...
 public:
  ...
  PairGranJKRHistory(class LAMMPS *);
  ~PairGranJKRHistory();
 ...
 protected:
  ...
  double **coefArea,**coefForce1,**coefForce2;
  ...
  virtual void deriveContactModelParams(int &ip, int &jp,double &meff,double &deltan, double &kn, double &kt, double &gamman, double &gammat, double &xmu, double &rmu, double &vnnr, double &area);
