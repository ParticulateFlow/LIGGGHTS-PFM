...
#include "fix_property_atom.h" //NP modified C.K.
#include "sph_kernels.h" //NP modified A.A.
#include "fix_sph.h" //NP modified A.A.
#include "update.h"//NP modified C.K.
...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     VX,VY,VZ,OMEGAX,OMEGAY,OMEGAZ,PROPERTYPERATOM}; //NP modified C.K.
...
void Set::command(int narg, char **arg)
{
  ...
  add    = 0; //NP modified C.K. //to switch on 'add' feature
  until  = 1; //NP modified C.K. // define number of timesteps the add feature to be used
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      set(Z);
      iarg += 2;
    } else if (strcmp(arg[iarg],"vx") == 0) {  //NP modified C.K.
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      set(VX);
      ...
      set(OMEGAX);
      iarg += 2;
    } else if (strcmp(arg[iarg],"omegay") == 0) {  //NP modified C.K.
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      set(OMEGAY);
      iarg += 2;
    } else if (strcmp(arg[iarg],"omegaz") == 0) {  //NP modified C.K.
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      set(OMEGAZ);
      ...
      if (!atom->rmass_flag && !atom->density_flag) //NP modified C.K.
        error->all(FLERR,"Cannot set this attribute for this atom style");
      ...
      set(DNAME);
      iarg += 2;
    } else if (strcmp(arg[iarg],"add") == 0){  //NP modified C.K.
      if (iarg+1 > narg)
      ...
      iarg +=2;
    } else if (strcmp(arg[iarg],"until") == 0){ //NP modified C.K.
     if (iarg+1 > narg)
      ...
      iarg +=2;
    } else if (strncmp(arg[iarg],"property/atom",13) == 0) { //NP modified C.K.
      if (iarg+1 > narg)
      ...
      iarg += (2+nUpdValues);
    } else if (strcmp(arg[iarg],"sphkernel") == 0) { //NP modified A.A.
      if (iarg+2 > narg) error->all(FLERR, "Illegal set command");
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (keyword == TYPE) {
      ...
      else if (keyword == VX) atom->v[i][0] = dvalue; //NP modified C.K.
      else if (keyword == VY) atom->v[i][1] = dvalue;
      else if (keyword == VZ) atom->v[i][2] = dvalue;
      else if (keyword == OMEGAX) atom->omega[i][0] = dvalue;  //NP modified C.K.
      else if (keyword == OMEGAY) atom->omega[i][1] = dvalue;  //NP modified C.K.
      else if (keyword == OMEGAZ) atom->omega[i][2] = dvalue;  //NP modified C.K.
    ...
    else if (keyword == PROPERTYPERATOM) { //NP modified C.K.
    ...
    // else set rmass to density directly
      ...
      if (atom->radius_flag && atom->radius[i] > 0.0)
      {
          ...
          if (domain->dimension == 2)
            atom->rmass[i] = MY_PI * atom->radius[i]*atom->radius[i] * atom->density[i]; //NP modified C.K.
          else
            atom->rmass[i] = 4.0*MY_PI/3.0 * atom->radius[i]*atom->radius[i]*atom->radius[i] * atom->density[i]; //NP modified C.K.
      }
      else if (atom->density_flag)//NP modified C.K.
        atom->density[i] = dvalue;
