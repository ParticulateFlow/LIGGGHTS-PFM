...
{
    //NP override MultiNodeMeshParallel default setting
...
void TriMeshPlanar::postInitialSetup()
{
    ...
    //NP postInitialSetup() is after deleteUnowned()
    if(this->isParallel())
...
void TriMeshPlanar::buildEdgeLists()
{
    ...
    for(int iSrf = 0; iSrf < nall; iSrf++)
    {
        ...
        //NP also includes check against self
        for(int jSrf = 0; jSrf < nall; jSrf++)
        {
            //NP skip element pair totally if jSrf has no active edges
            ...
            for(int iEdge = 0; iEdge < NUM_NODES; iEdge++)
            {
                ...
                {
                    //NP skip edge pair if jEdge not active
                    if(!edgeActive(jSrf,jEdge))
                        continue;

                    dist = edgeNodeDist(iSrf,iEdge,jSrf,jEdge);

                    //NP less than 6 edges stored
