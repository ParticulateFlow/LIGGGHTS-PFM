...
namespace LAMMPS_NS
{
  class TriMeshPlanar : public TriMesh
  {
      ...
      private:
        ...
        //NP need to store 2 nearest edges for each node at max for planar face
        //NP stores IDs of triangles and their edge index
        VectorContainer<int,2*NUM_NODES> &nearestActiveEdgeID_;
        VectorContainer<int,2*NUM_NODES> &nearestActiveEdgeIndex_;

        //NP stores min distance from any point in tri to any active edge
