...
{
  version = new char[strlen(LAMMPS_VERSION)+strlen(LIGGGHTS_VERSION)+100]; //NP modified C.K.

  sprintf(version,"Version %s based on LAMMPS %s",LIGGGHTS_VERSION,LAMMPS_VERSION); //NP modified C.K.
...
Universe::~Universe()
{
  ...
  delete []version; //NP modified C.K.
  if(universe_id) delete []universe_id; //NP modified C.K.
}
...
void Universe::reorder(char *style, char *arg)
{
  ...
  if (strcmp(style,"nth") == 0) {
    ...
    if (me == 0) {
      ...
      while (1) {
        if ((ptr = strchr(line,'#'))) *ptr = '\0'; //NP modified R.B.
        if (strspn(line," \t\n\r") != strlen(line)) break;
        if (!fgets(line,MAXLINE,fp))
          error->one(FLERR,"Unexpected end of -reorder file");
      }
