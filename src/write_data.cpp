...
void WriteData::command(int narg, char **arg)
{
  ...
  tag_offset = tag_max = 0; //NP modified C.K.
  bool tag_flag = false;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"tag_offset") == 0) { //NP modified C.K.
      if (iarg+2 > narg) error->all(FLERR,"Illegal write_data command");
...
void WriteData::atoms()
{
  ...
  atom->avec->pack_data(buf,tag_offset); //NP modified C.K.
...
void WriteData::velocities()
{
  ...
  atom->avec->pack_vel(buf,tag_offset); //NP modified C.K.
...
void WriteData::bonds()
{
  ...
  atom->avec->pack_bond(buf); //NP modified R.B.
